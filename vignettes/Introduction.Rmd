...
Use: install.packages("FragPipeToTPPR") if package has not been installed.
...
Path to FragPipe Output Folder (For this test we are using a simplified FragPipe output folder located inside the package: dummyFragPipepath).
...
4. Convert FragPipe output (from TMT-I tmt-report/ratio_protein_None.tsv) to TPP-R input (1DTPP analysis only). The function tmtitotppr takes two variables: FragPipe output folder and experimental variables (the ones we created in the step 1).I t will ouput the headers to be changed and the before and after header values. This function will only return the path to the configuration files created by the fucntion ( a file needed by TPP-R to connect input files TMT labels with temperatures.)
...
Complete steps 1 - 4 from the **From FragPipe to TP-MAP** section. In step 5 instead of running tpprNormOneDTPP(configpath,FPpath), the full TPPR workflow function will be used. remember to use the correct configutation file path and to pass the function a directory to dave ther esults (in this case 1DTPP-TPPR folder):

```{r From TPP-R to TP-MAP}
